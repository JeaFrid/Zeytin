...
brew update
brew install git curl wget openssl nginx python3
...
if [[ "$INSTALL_LIVEKIT" == "y" ]]; then
    ...
    if ! command -v docker &> /dev/null; then
        ...
        echo -e "${YELLOW}Press ENTER after Docker is installed...${NC}"
        read
    fi
    ...
    if [ "$(docker ps -aq -f name=zeytin-livekit)" ]; then
...
if [[ "$INSTALL_NGINX" == "y" ]]; then
    ...
    NGINX_CONF="/usr/local/etc/nginx/servers/zeytin.conf"
...
EOF"
    ...
    sudo /opt/certbot/venv/bin/certbot certonly --webroot -w /usr/local/var/www -d $DOMAIN_NAME --non-interactive --agree-tos -m $EMAIL_ADDR
