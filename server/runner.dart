...
''' + _kReset);
    ...
    print('5. ${_kCyan}Update Dependencies$_kReset');
    ...
    switch (choice) {
      case '1': await _startTestMode(); break;
      case '2': await _startLiveMode(); break;
      case '3': await _watchLogs(); break;
      case '4': await _stopServer(); break;
      case '5': await _updateDeps(); break;
      case '6': await _cleanDatabase(); break;
      case '0': exit(0);
      default: print('Invalid selection.'); sleep(Duration(seconds: 1));
    }
...
Future<void> _startTestMode() async {
  ...
  var process = await Process.start('dart', ['bin/server.dart'], mode: ProcessStartMode.inheritStdio);
  await process.exitCode;
}

Future<void> _startLiveMode() async {
  print('\n$_kCyan[BUILD] Compiling AOT binary...$_kReset');
  var res = await Process.run('dart', ['compile', 'exe', 'bin/server.dart', '-o', binaryPath]);
  ...
  print('$_kGreen[SUCCESS] Server started in background (PID recorded).$_kReset');
}
...
Future<void> _watchLogs() async {
  if (!File(logFile).existsSync()) File(logFile).createSync();
  var process = await Process.start('tail', ['-f', logFile], mode: ProcessStartMode.inheritStdio);
  await process.exitCode;
}

Future<void> _updateDeps() async {
  print('\n$_kCyan[UPDATE] Updating Dart dependencies (pub get)...$_kReset');
  ...
  if (exitCode == 0) {
    print('$_kGreen[SUCCESS] Dependencies updated successfully.$_kReset');
  } else {
    print('$_kRed[ERROR] pub get failed with exit code $exitCode$_kReset');
  }
}
